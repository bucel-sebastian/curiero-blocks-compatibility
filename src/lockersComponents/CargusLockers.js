...
function CargusLockers({ orderId, cartData, shippingRates }) {
  ...
  const handleLockerChange = async (selectedOption) => {
    ...
    await dispatchCart.applyExtensionCartUpdate({
      ...
      data: {
        action: "urgentcargus-set-selected-locker",
        order_id: orderId,
        locker: lockerData,
      },
    });
    ...
    >
      <div className="wc-block-components-checkout-step__heading">
        <h2
          ...
          aria-hidden="true"
        >
          {curieroBlocks.i18n.selectLockerTitle.urgentcargus} *
        </h2>
      </div>
      <Select
        unstyled
        ...
        value={lockers.find(
          (locker) =>
            ...
            cartData?.extensions?.[
              "CurieRO-blocks"
            ]?.urgentcargusSelectedLocker?.id.toString()
        )}

...
function InnoshipLockers({ orderId, cartData, shippingRates }) {
  ...
  const getLockers = async (city, state, country) => {
    ...
    data.data.lockers.forEach((locker) => {
      lockersData.push({
        value: locker.id,
        label: `${locker.name} - ${locker.addressText}, ${locker.localityName}, ${locker.countyName}`,
        id: locker.id,
        name: locker.name,
        address: locker.addressText,
        city: locker.localityName,
        county: locker.countyName,
      });
    });
  ...
  const handleLockerChange = async (selectedOption) => {
    ...
    await dispatchCart.applyExtensionCartUpdate({
      ...
      data: {
        action: "innoship-set-selected-locker",
        order_id: orderId,
        locker: lockerData,
      },
    });
    ...
    >
      <div className="wc-block-components-checkout-step__heading">
        <h2
          ...
          aria-hidden="true"
        >
          {curieroBlocks.i18n.selectLockerTitle.innoship} *
        </h2>
      </div>
      <Select
        unstyled
        ...
        value={lockers.find(
          (locker) =>
            ...
            cartData?.extensions?.[
              "CurieRO-blocks"
            ]?.innoshipSelectedLocker?.id.toString()
        )}
...
export default InnoshipLockers;

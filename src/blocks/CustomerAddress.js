...
const CustomerAddress = ({ addressFieldsConfig, defaultEditing = false }) => {
  const {
    ...
    return {
      ...
      invalidProps: Object.keys(billingAddress)
        .filter((key) => {
          return (
            key !== "email" &&
            store.getValidationError("billing_" + key) !== undefined
          );
        })
        .filter(Boolean),
    };
  ...
  const addressFieldKeys = Object.keys(defaultAddressFields);
  ...
  const renderAddressFormComponent = useCallback(
    ...
    [addressFieldKeys, addressFieldsConfig, billingAddress, onChangeAddress]
  );
